"""Prepare and export the pending baked-PBR presentation-room review build."""
...
def bake_irradiance(scene, objects, image, samples, device):
    ...
    if device == "CUDA":
        ...
        cuda_devices = [
            ...
            if cycle_device.type == "CUDA"
        ]
        if not cuda_devices:
            raise RuntimeError("cuda_device_not_found")
        for cycle_device in preferences.devices:
            cycle_device.use = cycle_device in cuda_devices
...
def main():
    args = arguments()
    if args.bake and args.device != SUPPORTED_BAKE_DEVICE:
        raise RuntimeError(f"bake_requires_{SUPPORTED_BAKE_DEVICE.lower()}")
    ...
    materials = prepare_materials(objects, image)
    if args.bake:
        bake_irradiance(bpy.context.scene, objects, image, args.samples, args.device)
        ...
        json.dumps(
            {
                ...
                "scale": args.scale,
                "device": args.device,
                "defaultIntensity": args.intensity,
